...
public class Box implements Iterable<Ball> {
    ...
    public Iterator<Ball> getBallsFromSmallest() {
        TreeSet<Ball> sortedBalls = new TreeSet<>(new ballsCompare());
        Iterator<Ball> containerIter = ballContainer.iterator();
        while (containerIter.hasNext()) {
            sortedBalls.add(containerIter.next());
        }
...
class ballsCompare implements Comparator<Ball>{
    public int compare(Ball ball1, Ball ball2) {
        if(ball1.getVolume() > ball2.getVolume()){
            return 1;
