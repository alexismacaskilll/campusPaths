...
//[p1]:S is the result of putting p1 at
// * the front of the RatPolyStack S.
  /**
  ...
  public RatPoly getNthFromTop(int index) {
    Stack<RatPoly> newStack = new Stack<>();
    //{inv: newStack[0...i] = polys[polys.length-1...polys.length-1-i] )
    for (int i = 0; i < index; i++) {
      newStack.push(polys.pop());
    }
    RatPoly nthFromTop = polys.peek();
    //{inv: polys[polys.length-1-index...polys.length-1-index+i] = newStack[newStack.length-1...newStack.length-1-i] )
    for (int i = 0; i < index; i++) {
      polys.push(newStack.pop());
    }
    return nthFromTop;
  }
